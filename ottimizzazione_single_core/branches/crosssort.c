...
int main(int argc, char **argv)
{
  ...
  size_t SIZE = (size_t) SIZE_DEFAULT;
  int  use_random = 0;
  int  iter = ITER;
  ...
  printf("allocate room for 4x %zu elements and initialize it..\n", SIZE);
  
							    /* allocate memory for data         */
  if ( (Ac = (int*)calloc(SIZE*4, sizeof(int))) == NULL )
    {
      printf ( "not enough memory (%llu GB) to allocate %zu elements\n",
  ...
  if ( use_random )
    {							    
      ...
      printf( "%zu random numbers generation.. ", SIZE*2 ); fflush(stdout);
      for (size_t cc = 0; cc < SIZE*2; cc++)
	Ac[cc] = rand() % TOP;
    }

  else
    {
      printf( "%zu predictable numbers generation.. ", SIZE*2 ); fflush(stdout);
      for (size_t cc = 0; cc < SIZE/2; cc++)
	Ac[cc] = 1;
      for (size_t cc = SIZE/2; cc < SIZE; cc++)
	Ac[cc] = 2;
...
//	PAPI_FLUSH_BUFFER;
	...
	if ( cc == 0 )	                                    /* check that actually A[i] > B[i]     */
	  {						    /* only if the code has been compiled  */
	    size_t fail = 0;				    /* with -DDEBUG, and after the         */
	    for (size_t jj = 0; jj < SIZE; jj++)		    /* first iteration only                */
	      if ( A[jj] < B[jj] )
		fail++;
	    if ( fail > 0 )
