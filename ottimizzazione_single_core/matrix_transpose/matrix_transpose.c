...
// check L1 and L2 size with sysctl hw.l1dcachesize hw.l2cachesize
...
int main(int argc, char **argv)
{
  ...
  dprintf(0, "max N will be %d, Nsteps will be %d, run mode: %s\n",
    ...
    if (mode == STRIDED_WRITE)
      fout = fopen("transpose_strided_write.dat", "w");
    else
      fout = fopen("transpose_contiguous_write.dat", "w");
    ...
    fprintf(fout, "# N  bandwidth_MBps  time_s\n");
  for (N = Nmin; N <= Nmax; )
    {
	...
	for (int cc = 0; cc < nruns; cc++)
      ...
      dprintf(0, "size: %d [ %2d nruns] time: %5.3g s [total: %5.3g s] bw: %g MB/s ",
	      _N_, nruns, timing/nruns, timing, (2.0 * size) / timing);
     
        fprintf(fout, "%d  %.6f  %.6e\n", _N_, (2.0 * size) / timing, timing / nruns);
